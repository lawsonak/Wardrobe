...
- `/api/ai/collection-shop` — "Shop for this trip / collection". **Three-stage pipeline** that sidesteps grounded search's stale-URL problem (Gemini's training-cutoff index returns dead fashion URLs). Stage 1: Gemini reads the collection metadata + `buildClosetSummary` + `getTripForecast` + `computePackingTargets` and returns 3-12 product **specs** (no URLs) tuned by a 0-100 closet-awareness slider — see `lib/ai/collectionShop.ts:specifyProductsForCollection`. Stage 2: each spec is run through Google Programmable Search (`lib/productSearch.ts`) against the curated retailer list configured in the user's CSE. Stage 3: every CSE hit is fetched via `lib/productMeta.ts`, dead/blocked/non-product pages are dropped, and the final card uses the real `og:image` + JSON-LD price. Orchestrated by `lib/ai/shopPipeline.ts`. Requires `GOOGLE_CSE_API_KEY` + `GOOGLE_CSE_ID` in addition to `GEMINI_API_KEY`; returns a clear "search isn't configured" error when CSE is missing.
...
| Shop for this trip / collection | `lib/ai/collectionShop.ts` (Stage 1 specs), `lib/ai/shopPipeline.ts` (orchestrator), `lib/productSearch.ts` (Google CSE), `app/api/ai/collection-shop/route.ts`, `app/(app)/collections/CollectionShop.tsx` |
