...
- `/api/mannequin` — per-user "personal mannequin": upload a photo (multipart `source` File) and the route runs a three-step Gemini 2.5 Flash Image pipeline — generate a faceless fashion-illustration body, generate a friendly cartoon head, then compose head + body into a single image that becomes the canonical mannequin. JSON `{ mode: "regenerate" }` re-runs the whole pipeline on the saved source. `DELETE` wipes back to the global default. See `lib/ai/mannequin.ts` (`generateMannequinFromPhoto`, `generateStylizedHead`, `composeBodyWithHead`) + `lib/mannequin.ts`.

**Face on the mannequin:** the head is baked into the mannequin PNG at generation time — Gemini composes body + cartoon head into one image. No CSS overlay, no separate head file. If the head generation or compose call fails, the headless body is saved instead so try-on still works (just faceless). To redo just the face, hit Regenerate (re-runs the whole pipeline; ~30-60s).
