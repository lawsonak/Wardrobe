...
| `CollectionShopItem` | A "considering buying this" product on a collection's shopping list | collectionId, name, brand, category, color, price, link, `imagePath` (product thumbnail saved under `<userId>/collection-shop/`), source (host), notes, **purchased** flag, plus per-product try-on cache (`tryOnImagePath`, `tryOnHash`, `tryOnGeneratedAt`). Distinct from `CollectionItem` (owned closet Item) and `WishlistItem` (global standalone wishlist). `POST /api/collections/[id]/shop-items` is content-type dispatched with three input shapes: (a) JSON `{ link }` runs `lookupWishlistProduct` + `saveRemoteImage`; (b) JSON `{ name, ...fields }` is the manual path used by the AI shop panel; (c) multipart with `image` runs `detectMultipleItems` to auto-crop AND auto-tag the photo (name / category / color / brand pulled from the detector's per-item `TagSuggestion`) — link can stay null and be added later. `PATCH .../[shopItemId]` edits `purchased`, `notes`, `link`, `name`, `brand`, `category`, `color`, `price` (each validated). `POST .../[shopItemId]/photo` REPLACES an existing card's photo (same auto-crop) and invalidates the cached try-on. `POST .../[shopItemId]/tryon` composes the single product onto the user's mannequin via `generateTryOn` — sidesteps the 5-garment cap since only one piece is sent. |
