...
- **Body-measurements feature — Phase E only.** Phases A-D have shipped: A = schema + `lib/measurements.ts` + `/api/measurements` + `/settings/measurements`; B = `assessFit` garment fit badge on the item page (no AI); C = `measurementsSummary` injected into Today's Suggestion + Shop-for-trip prompts (deliberately not product/wishlist lookup — fixed output schema, no size slot); D = `proportions` appended to the mannequin generation prompt (`mannequin.ts`) so the figure matches real body — the try-on inherits this via the mannequin image and is NOT given measurements directly (its prompt's whole job is "keep the body exactly as image 1"). Remaining: **E** — `/api/ai/estimate-measurements` + an optional "✨ Estimate from a photo" button that uses a known-size reference object (credit card / US Letter / dollar bill) for scale, returning an editable draft.
