...
- `/api/mannequin` — per-user "personal mannequin": upload a photo (multipart `source` File) and Gemini 2.5 Flash Image generates a stylized fashion-illustration of a neutral dress-form matching that body type. JSON `{ mode: "regenerate" }` re-runs on the saved source. `{ mode: "regenerate-face" }` redoes only the head overlay; `{ mode: "remove-face" }` deletes just the head overlay (keeps the mannequin). `DELETE` wipes back to the global default. See `lib/ai/mannequin.ts` + `lib/mannequin.ts`.

**Face overlay (optional):** the mannequin upload also generates a stylized head crop (`mannequin-head.png`) — Gemini Flash Image redrawing just the user's head in the same illustration style as the mannequin. The try-on UIs (`TryOnView`, `TodaysOutfitCard`) stack it on top of the AI-rendered body via CSS positioning at `headBBox` (normalized 0..1 in `mannequin.json`). No AI in the merge — pure visual layering. Removing the overlay (Settings → "Remove face") deletes only the head PNG; the mannequin stays.
