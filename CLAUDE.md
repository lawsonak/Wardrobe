...
| Collection shopping list (paste product links → saved cards) | `app/(app)/collections/CollectionShopItems.tsx` (paste box + sequential per-link queue + cards), `app/api/collections/[id]/shop-items/route.ts` (POST one link → row, reusing `lookupWishlistProduct` + `saveRemoteImage`), `app/api/collections/[id]/shop-items/[shopItemId]/route.ts` (PATCH purchased / DELETE), `lib/remoteImage.ts` (`saveRemoteImage` — SSRF-guarded download + sharp resize). Rendered inside `CollectionEditor`; adds/removes persist immediately (independent of the metadata "Save changes" button). |
