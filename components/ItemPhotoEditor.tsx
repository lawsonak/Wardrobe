...
//   2. Re-run background removal on the existing photo.
//   3. Drop the bg-removed variant.
//
// Label / tag photos used to live here too but moved to ItemLabels
// (parallel to ItemAngles) when an item gained the ability to carry
// many labels — that component does its own file picker and reuses
// the shared /api/items/[id]/photos endpoint.
export default function ItemPhotoEditor({
  ...
  // `accept="image/*"` lets the OS prompt show both "Take Photo" and
  // "Photo Library" on mobile, so a separate camera button isn't
  // needed.
