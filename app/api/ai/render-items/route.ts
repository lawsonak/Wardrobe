...
// Multi-image generation can take 30-60s.
export const maxDuration = 90;

const UPLOAD_ROOT = path.join(process.cwd(), "data", "uploads");

// FNV-1a, plenty for naming a cache file.
...
// used by the dashboard's "Today's outfit" card to get a polished
// composition without needing a saved Outfit row. Results are cached
// to disk keyed by sorted itemIds + mannequin mtime, so re-running
// the same pick is free.
export async function POST(req: NextRequest) {
  ...
  // Cache lookup. Key invalidates when items change (sort to ignore
  // pick order) or when the user regenerates their mannequin.
  ...
  try {
    ...
    /* not cached — fall through to render */
  }
