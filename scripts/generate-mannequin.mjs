#!/usr/bin/env node
// Generate the canonical photoreal mannequin image used by the AI
// virtual try-on feature. Calls Gemini 2.5 Flash Image with a fixed
// prompt and writes the result to public/mannequin/base.png. Bumping
// the `id` in base.json invalidates every cached try-on (the route
// includes it in the cache hash), so when you re-run this script and
// want the change to propagate immediately, also bump the id.
//
// Usage:
//   GEMINI_API_KEY=... node scripts/generate-mannequin.mjs
//   GEMINI_API_KEY=... node scripts/generate-mannequin.mjs --id mq-v2
